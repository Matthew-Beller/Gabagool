    ...
    for video_file_name in os.listdir(intial_directory):
        if(os.path.isfile(os.path.join(intial_directory, video_file_name))):
            ...
            if(subtitle_file != None):
                found_matches_file = find_output_file(video_file_name, intial_directory)
...
def find_output_file(video_file_name, intial_directory = None):
...
if(args.action == 'Single'):
    ...
    output_file = find_output_file(os.path.basename(source_file_video))
    find_matching_entries(source_file_subtitle, key_phrase, output_file, source_file_video)
elif(args.action == 'Batch'):
    directory_found = False
